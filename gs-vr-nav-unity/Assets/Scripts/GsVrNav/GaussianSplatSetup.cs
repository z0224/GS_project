    ...
    public sealed class GaussianSplatSetup : MonoBehaviour
    {
        ...
        private void Start()
        {
            if (applyENUToUnityRotation)
            {
                ...
                // CN: Python ENU 是 (East, North, Up)，Unity 是 (X, Y, Z)=(Right, Up, Forward)。
        ...
        private void ConfigureRendererBestEffort()
        {
            ...
            // CN: 第三方 GaussianSplatRenderer 不同版本字段名可能不同，所以用反射做宽松配置。
            // EN: Different GaussianSplatRenderer versions may expose different field names, so reflection keeps setup flexible.
